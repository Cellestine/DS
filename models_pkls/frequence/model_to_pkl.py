...
class ColumnSelector(BaseEstimator, TransformerMixin):
    """
    Sélectionne les colonnes spécifiées dans les données d'entrée.

    Args:
        selected_columns (list): Liste des colonnes à conserver.
    """
...
class ManualCountEncoder(BaseEstimator, TransformerMixin):
    """
    Encode les colonnes catégorielles en fonction de leur fréquence d'apparition.
    """
...
class ScalerWrapper(BaseEstimator, TransformerMixin):
    """
    Applique une normalisation (StandardScaler) sur les colonnes numériques.
    """
...
pipeline = Pipeline(
    steps=[
        ...
        (
            "missing",
            MissingValueFiller(num_cols=NUMERICAL_COLUMNS, cat_cols=CATEGORIAL_COLUMNS),
        ),
